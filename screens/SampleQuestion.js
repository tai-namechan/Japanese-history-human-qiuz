...
export default function SampleQuestion(props) {
  ...
  const [disabled, isDisabled] = useState({0: false, 1: false, 2: false, 3: false})
  ...
  function isEmpty(questionOptions) {
  ...
  const fetchQuestions = async () => {
    try {
      await axios
        .get(`http://rekishino-kabe-api.herokuapp.com/api/question/${number}`)
        .then((res) => {
    ...
    <ThemeProvider theme={theme}>
            ...
            {/* <View style={styles.buttons}>
              {question.questionOptions.map((questionOption, i) => (
                <Button
                  ...
                  // disabled={diabled[status]}
                />
              ))}
            </View> */}
            <Button title="わかった！" onPress={toggleOverlay} />
          </View>
        </ImageBackground>
      </View>
      <Overlay isVisible={visible}>
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-around" }}>
          <Button title="閉じる" onPress={toggleOverlay} />
          <Button
            title="回答"
            disabled={!isValid}
